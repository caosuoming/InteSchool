...
export interface GradeElectiveScoreSegmentRow {
  ...
  thresholdCounts: Record<number, number>;
}
...
export interface GradeElectiveScoreSegmentSubjectReport {
  ...
  totalThresholdCounts: Record<number, number>;
}
...
function rulesForSubject(settings: GradeExamSettings, subject: string): GradeBandRule[] {
  return settings.assignmentRules[subject]?.length
    ? settings.assignmentRules[subject]
    : DEFAULT_ASSIGNMENT_RULES;
}

function gradeLabel(value: number, rules: GradeBandRule[]): string | null {
...
export function buildGradeElectiveScoreSegmentReport(
  ...
  const subjects = GRADE_ELECTIVE_SCORE_SEGMENT_SUBJECTS
    ...
    .map((subject) => {
      ...
      const rules = rulesForSubject(settings, subject);
      ...
      const rows = (classOptions.classOrder || [])
        ...
        .map((classId) => {
          ...
          values.forEach((value) => {
            const label = gradeLabel(value, rules);
          ...
          return {
            ...
            gradeCounts,
            thresholdCounts: Object.fromEntries(thresholds.map((threshold) => [
              threshold,
      ...
      return {
        ...
        rows,
        totalCandidateCount,
        totalGradeCounts: Object.fromEntries(gradeLabels.map((label) => [
          label,
          rows.reduce((sum, row) => sum + (row.gradeCounts[label] || 0), 0),
        ])),
        totalThresholdCounts: Object.fromEntries(thresholds.map((threshold) => [
          threshold,
          rows.reduce((sum, row) => sum + (row.thresholdCounts[threshold] || 0), 0),
        ])),
