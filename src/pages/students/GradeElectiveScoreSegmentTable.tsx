...
import {
  ...
  GRADE_ELECTIVE_SCORE_SEGMENT_SUBJECTS,
  resolveGradeElectiveScoreThresholds,
} from "@/lib/grade-elective-score-segment";
import { exportGradeElectiveScoreSegmentReport } from "@/lib/grade-spreadsheet";
...
export function GradeElectiveScoreSegmentTable({
  exam,
  settings,
  template,
  ...
  onChange,
  onAutoSave,
}: GradeElectiveScoreSegmentTableProps) {
  ...
  const report = useMemo(() => buildGradeElectiveScoreSegmentReport(
    exam,
    effectiveTemplate,
    context,
    settings,
    ...
    <Card className="overflow-hidden p-0">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <div className="mt-0.5 text-xs text-ink-500">
              统计化学、生物、政治、地理各班实考人数、A-E 等级人数和累计达分人数；任课教师来自当前年级配置。
            </div>
          </div>
        </div>
      ...
      {adjusting && (
        <div className="border-b border-ink-200 bg-ink-50/60 px-5 py-4">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {GRADE_ELECTIVE_SCORE_SEGMENT_SUBJECTS.map((subject) => (
              <Input
                key={subject}
                label={`${subject}分数段`}
                defaultValue={resolveGradeElectiveScoreThresholds(effectiveTemplate, subject).join("，")}
                onBlur={(event) => {
                  updateThresholds(subject, event.target.value);
                  void autoSave();
                }}
                placeholder="例如 90，80，70"
              />
            ))}
          </div>
          <div className="mt-2 text-[11px] text-ink-400">
            A-E 等级按当前赋分规则统计；累计分数段可独立调整。{autoSaving ? "正在自动保存…" : "失焦后自动保存到当前届年级。"}
          </div>
        </div>
      )}
      ...
      {report.subjects.length === 0 ? (
        <EmptyState
          ...
          title="当前考试没有表四所需选修成绩"
          description="请先导入化学、生物、政治或地理成绩。"
        />
      ) : (
          ...
          {report.subjects.map((subjectReport) => (
            <section key={subjectReport.subject} className="overflow-x-auto">
              <div className="mb-3 flex min-w-max items-end justify-between gap-4 px-1">
                ...
                <div className="shrink-0 text-sm font-medium text-ink-600">
                  实考 {subjectReport.totalCandidateCount} 人 · {report.reportDate.replace(/-/g, ".")}
                </div>
              </div>
              <table className="w-max min-w-full table-auto border-collapse text-[11px]">
                <thead>
                  <tr className="bg-ink-50 text-ink-700">
                    ...
                    <th className="whitespace-nowrap border border-ink-300 px-2 py-1.5 text-center font-semibold">任课教师</th>
                    ...
                    {subjectReport.gradeLabels.map((label) => (
                      <th key={label} className="whitespace-nowrap border border-ink-300 px-2 py-1.5 text-center font-semibold">{label}</th>
                    ))}
                ...
                <tbody>
                    ...
                    <tr key={row.classId} className="bg-paper">
                      ...
                      {subjectReport.thresholds.map((threshold) => (
                        <td key={threshold} className="whitespace-nowrap border border-ink-300 px-2 py-1.5 text-center font-semibold tabular-nums">
                          {row.thresholdCounts[threshold] || 0}
                        </td>
                      ))}
                    </tr>
                  ))}
                  <tr className="bg-ink-50/70 font-semibold">
                    ...
                    {subjectReport.thresholds.map((threshold) => (
                      <td key={threshold} className="border border-ink-300 px-2 py-1.5 text-center tabular-nums">{subjectReport.totalThresholdCounts[threshold] || 0}</td>
                    ))}
                  </tr>
                </tbody>
              </table>
