...
function strokeTouchesPoint(stroke: PrepAnnotationStroke, point: { x: number; y: number }): boolean {
  const radius = stroke.tool === "highlighter" ? 0.035 : 0.022;
  return stroke.points.some((current, index) => {
    const previous = stroke.points[Math.max(0, index - 1)];
    return distanceToSegment(point, previous, current) <= radius;
  });
}
...
export function PrepBoardReviewModal({
  ...
  const [activeTool, setActiveTool] = useState<AnnotationTool | null>(toolOptions[0]);
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      setIsFullscreen(Boolean(
        reviewRef.current && document.fullscreenElement === reviewRef.current,
      ));
    };
  ...
  useEffect(() => {
    if (!open && reviewRef.current && document.fullscreenElement === reviewRef.current) {
  ...
  const handlePointerDown = (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (!activeTool) {
      ...
      setStrokes((current) => current.filter((stroke) => (
        stroke.createdBy !== teacher.id || !strokeTouchesPoint(stroke, point)
      )));
      return;
    }
  ...
  const handleToggleFullscreen = async () => {
    try {
      if (document.fullscreenElement === reviewRef.current) {
        await document.exitFullscreen();
        setIsFullscreen(false);
      } else if (reviewRef.current) {
        await reviewRef.current.requestFullscreen();
    ...
    >
      {item ? (
        <div
          ref={reviewRef}
          className={cn(
            "flex h-[74vh] min-w-0 flex-col overflow-hidden rounded-xl border border-ink-100 bg-mist/60",
            isFullscreen && "h-screen rounded-none border-0 bg-mist p-4",
          )}
        >
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-ink-100 bg-paper px-4 py-2">
            <span className="mr-1 text-xs font-medium text-ink-500">批注</span>
            {toolOptions.map((option) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "group flex h-12 min-w-12 flex-col items-center justify-center gap-0.5 rounded-lg border px-2 transition-colors",
                    active ? "border-gold-400 bg-gold-50 shadow-sm" : "border-transparent hover:border-ink-100 hover:bg-mist",
                  )}
                >
                  <Icon className="h-6 w-6 -rotate-12" style={{ color: option.hex }} />
                  <span className="h-1.5 w-7 rounded-full border border-black/10" style={{ backgroundColor: option.hex }} />
                </button>
              );
            })}
            <button
              ...
              className={cn(
                "flex h-12 min-w-12 flex-col items-center justify-center gap-0.5 rounded-lg border px-2 transition-colors",
                !activeTool ? "border-gold-400 bg-gold-50 shadow-sm" : "border-transparent hover:border-ink-100 hover:bg-mist",
              )}
            >
              <Eraser className="h-6 w-6 -rotate-12 text-ink-600" />
              <span className="h-1.5 w-7 rounded-full border border-ink-200 bg-paper" />
            </button>
            ...
            <div className="ml-auto hidden text-xs text-ink-500 lg:block">
              当前显示 {strokes.length} 条批注，其中 {strokes.filter((stroke) => stroke.createdBy === teacher.id).length} 条由你添加。
            </div>
            <Button variant="gold" size="sm" loading={saving} onClick={() => void handleSave()}>
          ...
          <div ref={previewRef} className="relative min-h-0 flex-1 overflow-hidden bg-white">
            ...
            <svg
              ...
              className={cn(
                ...
                activeTool ? "cursor-crosshair" : "cursor-cell",
              )}
